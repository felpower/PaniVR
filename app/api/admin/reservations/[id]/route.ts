...
export async function PATCH(request: NextRequest, context: RouteContext<'/api/admin/reservations/[id]'>) {
  ...
  try {
    if (notify) {
      ...
      const emailContent = confirmationEmail({ name, date, slot, players: Number(current.players || 0), occasion: String(current.occasion || ''), reference: bookingReference(date, slot) });
