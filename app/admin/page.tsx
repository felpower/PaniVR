...
export default async function AdminPage() {
  ...
  return (
    <main className="admin-page">
      ...
      <div className="admin-content"><div className="admin-title"><div><p className="admin-kicker">Übersicht</p><h1>Reservierungen</h1></div><div className="admin-title-links"><Link href="/admin/team">Team verwalten</Link><Link href="/"><ArrowLeft size={16} /> Website ansehen</Link></div></div>{loadError && <p className="admin-error">{loadError}</p>}<AdminReservations initialReservations={reservations} /></div>
    </main>
  );
}
