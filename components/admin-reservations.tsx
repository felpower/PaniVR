...
export function AdminReservations({ initialReservations }: { initialReservations: AdminReservation[] }) {
  ...
  const upcoming = reservations.filter((item) => item.date >= today && item.status === 'confirmed').length;
  const people = reservations.filter((item) => item.status === 'confirmed').reduce((sum, item) => sum + item.players, 0);
  ...
  return (
    <>
      ...
      <section className="admin-panel">
        <div className="admin-panel-head"><div><p>Reservierungen</p><h2>Alle Buchungen</h2></div><div className="admin-tools"><label><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Name, E-Mail, Datum …" /></label><select value={filter} onChange={(event) => setFilter(event.target.value)} aria-label="Nach Status filtern"><option value="all">Alle Status</option><option value="confirmed">Bestätigt</option><option value="completed">Erledigt</option><option value="cancelled">Storniert</option></select></div></div>
        {error && <p className="admin-error" role="alert">{error}</p>}
        {filtered.length ? (
          <div className="admin-table-wrap"><table><thead><tr><th>Termin</th><th>Kontakt</th><th>Gruppe</th><th>Notiz</th><th>Status</th></tr></thead><tbody>{filtered.map((reservation) => <tr key={reservation.id}><td><strong>{displayDate(reservation.date)}</strong><span><Clock3 size={13} /> {reservation.slot} Uhr</span></td><td><strong>{reservation.name}</strong><a href={`mailto:${reservation.email}`}><Mail size={12} />{reservation.email}</a><a href={`tel:${reservation.phone}`}><Phone size={12} />{reservation.phone}</a></td><td><strong>{reservation.players} Personen</strong><span>{reservation.occasion || '–'}</span></td><td className="admin-notes">{reservation.notes || '–'}</td><td><select className={`status-select status-${reservation.status}`} value={reservation.status} disabled={saving === reservation.id} onChange={(event) => updateStatus(reservation.id, event.target.value)}><option value="confirmed">Bestätigt</option><option value="completed">Erledigt</option><option value="cancelled">Storniert</option></select></td></tr>)}</tbody></table></div>
